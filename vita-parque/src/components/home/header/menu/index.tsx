...
export default function Menu() {
  ...
  useEffect(() => {
    function outsideClose(event: any) {
      if (
        ...
        event.target !== hamburguer.current
      ) {
        setToggleMenu(false);
      }
    }
  ...
  return (
    <>
      ...
      <nav
        className={
          toggleMenu ? `${styles.menu} ${styles.menu_active}` : styles.menu
        }
        ref={menuRef}
      >
        <ul>
      ...
      <button
        className={
          toggleMenu
            ? `${styles.menu__hamburguer} ${styles.menu__hamburguer_active}`
            : `${styles.menu__hamburguer}`
        }
        onClick={() => {
