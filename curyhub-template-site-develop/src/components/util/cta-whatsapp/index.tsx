...
export default function CtaWhatsapp({ children }: ICta) {
  const [nomeError, setNomeError] = useState<any>(null);
  const [emailError, setEmailError] = useState<any>(null);
  const [telError, setTelError] = useState<any>(null);
  ...
  function handle_post_anapro(event: any) {
    ...
    form.forEach((value, key) => {
      ...
      key === "email" && value !== "" && validateEmail(value) ? (data.email = value) : null;
      key === "tel" && value !== "" && validateCel(value) && validateDDI(value) ? (data.tel = value) : null;
    });

    // Mensagens de erro
    !data.nome ? setNomeError("Preencha o campo nome corretamente.") : setNomeError(null);
    !data.email ? setEmailError("Preencha o campo e-mail corretamente.") : setEmailError(null);
    !data.tel ? setTelError("Preencha o campo telefone corretamente.") : setTelError(null);
  ...
  return (
    <div className="whatsapp-MOP19072021 active">
      <div className="whatsapp-MOP19072021__item">
        ...
        <form action="#" className="whatsapp-MOP19072021__form" onSubmit={handle_post_anapro}>
          ...
          <div className="whatsapp-MOP19072021__form-column">
            ...
            {loading ? <input type="submit" value="Aguarde..." /> : <input type="submit" value="Iniciar conversa" />}
          </div>
        </form>
      </div>
    </div>
